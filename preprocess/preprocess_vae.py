...
def worker(gpu, cfg):
    ...
    if cfg.world_size == 1:
        ...
        if num_samples is not None and num_samples > 0:
            img_dataset = torch.utils.data.Subset(img_dataset, range(min(num_samples, len(img_dataset))))
            logging.info(f"Using {len(img_dataset)} samples (limited by --num_samples)")
        ...
        logging.info('Start the preprocess loop (single GPU)')
    ...
    # ── 多卡分支（原代码不动，仅当 world_size > 1 时进入）──
    torch.cuda.set_device(gpu)
    ...
    latent_save_root = cfg.latent_save_root
    logging.info('Start the preprocess loop (multi-GPU)')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--latent_save_root', type=str, default="E:/dataset/DiffSynth-Studio/OpenGPT-4o-Image/sd-vae-ft-mse_Latents_1024img_npz", help="Root directory path to save or load the latents")
    parser.add_argument('--num_samples', type=int, default=500,
                        help="Number of samples to process. If not set, process all data.")
