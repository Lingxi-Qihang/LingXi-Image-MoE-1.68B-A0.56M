...
def precompute_rope_1d(pos, dim, theta=10000.0):
    """Standard 1D RoPE frequencies."""
    device = pos.device
    freqs = 1.0 / (theta ** (torch.arange(0, dim, 2, device=device) / dim))
    angles = pos.unsqueeze(-1).float() * freqs.unsqueeze(0)
    cos = angles.cos().repeat_interleave(2, dim=-1)
    sin = angles.sin().repeat_interleave(2, dim=-1)
    return cos, sin


def precompute_2d_rope(h_pos, w_pos, head_dim, theta=10000.0):
    """2D RoPE: first half for height, second half for width."""
    half_d = head_dim // 2
    h_cos, h_sin = precompute_rope_1d(h_pos, half_d, theta)
    w_cos, w_sin = precompute_rope_1d(w_pos, half_d, theta)
...
def apply_rotary_emb(x, cos, sin):
    """Apply RoPE (pairwise rotation) to x."""
    if cos.dim() == 2:
    ...
    cos_pair = cos.reshape(*cos.shape[:-1], D // 2, 2)
    sin_pair = sin.reshape(*sin.shape[:-1], D // 2, 2)
    x_even = x_pair[..., 0]
    x_odd = x_pair[..., 1]
    c = cos_pair[..., 0]
    s = sin_pair[..., 0]
